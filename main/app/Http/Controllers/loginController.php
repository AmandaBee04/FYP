...
class LoginController extends Controller
{
    public function login(LoginRequest $request)
    {
        ...
        $expirationTimestamp = now()->addHours(24)->timestamp;
        Log::info('Expiration timestamp: ' . $expirationTimestamp);
        // Create JWT payload
        $payload = [
            'sub' => $user->id,
            'data' => $encryptedToken,
            'exp' => $expirationTimestamp// Optional expiration time
        ];
        ...
        return response(['message' => 'Login successful', 'jwt' => $jwt, 'role' => $role, 'id' => $id])->withCookie($cookie);
    }
}